...
class CodeGNNDualFcInfo:
    ...
    def create_model(self):
        ...
        met_input = Input(shape=(self.metlen,))  # (None,17)input_6 method
        ...
        for i in range(self.config['asthops']):
            top2down = GCNLayer(100)([top2down, edge_input])  # 1:(None, 100, 100) 2:(None, 100, 100) gcn_layer_1->gcn_layer_3
        ...
        context = concatenate([scontext,tcontext, decout, acontext, mcontext])  # (None, 13, 768)
        ...
        # # pointer for tdat
        # concate_2 = concatenate([tcontext, decout, de])  # (None, 13, 612)
        # p_gen_2 = TimeDistributed(Dense(1))(concate_2)  # (None, 13, 1)
        # p_gen_2 = Lambda(lambda x: K.mean(x, axis=1))(p_gen_2)  # compute the mean the(None, 1)
        # p_gen_2 = Activation('sigmoid')(p_gen_2)  # (None, 1)
        # attn_concate_2 = Lambda(lambda x: K.mean(x, axis=1))(
        #     tattn)  # [batch_size, dec_length, enc_length] -> [batch_size, enc_length](None, 50)

        if self.config['use_pointer']:
            #tdat_final_dists = self._calc_final_dist(out, attn_concate_2, p_gen_2, tdat_input)
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists, p_gen, type_input):
        ...
        final_dists = AddEpsilon(final_dists) # add a minimum
